...
export async function POST(request: NextRequest) {
    try {
        ...
        // Extract data
        const permitData = {
            id: randomUUID().substring(0, 12), // Original used hex[:12] of uuid
            ...
            issueDateIso: formData.get('issueDateIso') ? new Date(formData.get('issueDateIso') as string) : null,
            expiryDate: formData.get('expiryDate') as string,
            expiryDateIso: formData.get('expiryDateIso') ? new Date(formData.get('expiryDateIso') as string) : null,
            status: formData.get('status') as string,
        };

        await db.insert(permits).values(permitData);

        // Redirect to home after creation (original behavior)
